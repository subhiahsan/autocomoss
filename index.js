...
function main() {
    ...
    jedawaktu = process.env.JEDA
    waktu = jedawaktu.split(", ")
    pilihwaktu = Math.floor(Math.random() * waktu.length);
    waktujadi = waktu[pilihwaktu]

    function mskemins(waktujadi) {
        var minutes = Math.floor(waktujadi / 60000)
        return minutes
    }
    ...
    const makeCommit = async (git, n) => {
        ...
        jsonfile.writeFile(process.env.DATA_PATH, data, () => {
            git
                ...
                .commit(DATE, {
                    '--date': DATE
                }, makeCommit.bind(this, git, --n))
                .push(['-u', 'origin', process.env.BRANCH_NAME], () => {
                    console.log('\nProcessing => ', DATE, 'DONE')
                    doneCommitAmount++
